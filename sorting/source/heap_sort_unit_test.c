...
void unit_test_heap_sort(){
  test_heap_sort_on_normal_array();
  test_heap_sort_on_null_array();
  test_heap_sort_on_ordered_array();
  test_heap_sort_on_equals_array();
  test_heap_sort_on_reversed_array();
}
...
void test_heap_sort_on_ordered_array() {
  ...
  heap_sort((void**) array, 9, compare_int_ptr);
...
void test_heap_sort_on_reversed_array() {
  int* array[10];
  for(int i=9; i>=0; i--) 
...
void test_heap_sort_on_normal_array() {
  int* array[10];
  array[0] = new_int(10);
  array[3] = new_int(7);
  array[1] = new_int(9);
